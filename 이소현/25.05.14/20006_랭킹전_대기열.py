...
1 초	256 MB	8013	3268	2453	38.961%
문제
...
player_cnt, max_player_cnt = map(int, input().split())
players = []
for _ in range(player_cnt):
    level, id = input().split()
    players.append((int(level), id))
...
for player in players:
    if rooms:
        is_player_checkin = False
        for i in range(len(rooms)):
            if len(rooms[i]) >= max_player_cnt:
                continue  
            elif rooms[i][0][0] -10 <= player[0] and rooms[i][0][0] +10 >= player[0] :
                rooms[i].append((player))
                is_player_checkin = True
                break
        if not is_player_checkin:
            rooms.append([player])    
...
for room in rooms:
    ...
    if len(room) == max_player_cnt:
        print("Started!")

    else:
        print("Waiting!")
    for player_level, player_id in sorted_room:
        print( player_level, player_id )
