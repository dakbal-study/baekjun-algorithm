...
원래 회전 초밥은 손님이 마음대로 초밥을 고르고, 먹은 초밥만큼 식대를 계산하지만, 벨트의 임의의 한 위치부터 k개의 접시를 연속해서 먹을 경우 
할인된 정액 가격으로 제공한다.
...
# k개 연속 선택을 위한 belt를 2배로 만들어도 됨(단, N <= 3만이니 슬라이딩만 해도 충분)
count = Counter()
...
# 쿠폰 초밥은 무조건 하나 추가로 제공받으므로, 윈도우에 이미 포함되어 있는지 관계없이 넣어봄
max_type = len(count) + (0 if count[c] else 1)

for i in range(1, N):
    # 앞에서 빠질 초밥
    out_sushi = belt[i-1]
    count[out_sushi] -= 1
    # out_sushi 초밥의 개수를 하나 줄였을 때 만약 그 종류의 초밥이 더 이상 윈도우에 남아있지 않다면(0개라면) 딕셔너리에서 해당 초밥 종류를 완전히 삭제
    # len(count)로 종류의 개수를 셀 때 개수가 0인 항목은 제외하기 위함
    if count[out_sushi] == 0:
        del count[out_sushi]

    # 뒤에 추가될 초밥 (원형)
    in_sushi = belt[(i + k - 1) % N]
    count[in_sushi] += 1

    # 쿠폰 초밥이 이미 포함되어 있으면 +0, 아니면 +1
    now = len(count) + (0 if count[c] else 1)
    if now > max_type:
        max_type = now
